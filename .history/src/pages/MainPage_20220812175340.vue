<template>
  <div>
    <div class="addpostbtnbox">
      <my-button class="add-post__btn" v-on:click="warn('Форма не может быть отправлена.', $event)">
        Добавить пост
      </my-button>
      <input
        class="post-search"
    ...
    <my-modal v-model="isModalShow" :isModalShow="isModalShow">
...
export default {
  ...
  computed: {
    sortedPosts() {
      ...
      //   ),
      // ];
      return postsCopy.sort((post1, post2) =>
          post1[this.selectedSort]?.localeCompare(post2[this.selectedSort])
        );
    },
  ...
  methods: {
    ...
    showModal() {
      console.log('add button clicked!!!!', this.isModalShow)
