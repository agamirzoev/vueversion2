...
export default {
  ...
  computed: {
    sortedPosts() {
      ...
      console.log(post1[this.selectedSort]?.localeCompare(post2[this.selectedSort]))
  ...
  watch: {
    ...
    sortedAndSearchedAndPaginatedPosts() {
      ...
      isPostLoading() {
      this.isPostLoading = this.posts = this.$store.state.post.isPostLoading;
      console.log(this.isPostLoading)
    }
  },
  async mounted() {
    ...
    console.log(this.isPostLoading)
    console.log(this.$store.state.post.isPostLoading)
  },
  methods: {
    createPost(post) {
      this.posts.push({
